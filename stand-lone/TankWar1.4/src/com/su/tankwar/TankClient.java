...
/**
 * 解决 missile 存在 arraylist中，越来越多，内存会耗尽，
 * solution： 判断missile out of the frame 就remove
 * 两种方法：
 * 1， 在missile类中增加一个boolean live = true， 然后 通过 get 方法获得这个值，
 * 当在missile类判断missile 出界， live = false； 然后在tankClient 的 paint方法中判断 对每个missile通过get 方法
 * 得到live的值判断是否false-》 remove
 * 
 * 2， 向添加missile一样，在创建missile时给missile类中传入 tankClient的引用，在missile类中，判断missile出界了，就使用tankClient的引用
 * remove这个missile
 * 
 * 这里使用第二种方法：
 *
...
public class TankClient extends Frame{
	...
	Tank tank = new Tank(50, 50, this); // instantiate a tank ++++++++
