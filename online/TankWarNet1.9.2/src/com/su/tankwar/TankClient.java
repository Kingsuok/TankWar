 ...
 *NetClient 是TankClient的一个功能模块，所以按理说是因该是TankClient的内部类，
 *考虑到练习外部类的目的，这里进行一个TankClient的使用外部类，对网络的操作进行控制
 *TankClient中有一个属性，netClient
 *
 *实际中所有的网络传输，现在都已经使用tcp，几乎不再使用udp了，这里只是练习一下，udp，TCP相对于UDP更简单
