...
/**
 *修正 给可以netClient设置server 的 ip 和 port 和 server 的udp port
 * @author su
 *
 */
public class TankClient extends Frame{
	...
	private String serverTCP_IP;
	private int serverTCP_PORT;
	private int serverUDP_PORT;
	private int clientUDP_PORT;
	...
	private class ConnectionDialog extends Dialog{
		...
		private TextField serverTcpIP = new TextField(12);
		private TextField serverTcpPort = new TextField(4);
		private TextField serverUdpPort = new TextField(4);
		...
		public ConnectionDialog() {
			...
			String tcpIPValue = PropertyManager.getProperty("SERVER_TCP_IP");
			serverTcpIP.setText(tcpIPValue);
			this.add(serverTcpIP);
			
			this.add(new Label("SERVER-TCP-PORT:"));
			String tcpPortValue = PropertyManager.getProperty("SERVER_TCP_PORT");
			serverTcpPort.setText(tcpPortValue);
			this.add(serverTcpPort);
			
			this.add(new Label("SERVER-UDP-PORT:"));
			String serverUdpPortValue = PropertyManager.getProperty("SERVER_UDP_PORT");
			serverUdpPort.setText(serverUdpPortValue);
			this.add(serverUdpPort);
			
			this.add(new Label("UDP-PORT:"));
			//get the updPort
			String clientUdpPortValue = PropertyManager.getProperty("CLIENT_UDP_PORT");
			clientUdpPort.setText(clientUdpPortValue);
			...
			button.addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent e) {
					...
					serverTCP_IP = serverTcpIP.getText().trim();
					serverTCP_PORT = Integer.parseInt(serverTcpPort.getText().trim());
					serverUDP_PORT = Integer.parseInt(serverUdpPort.getText().trim());
					int UDP_PORT = Integer.parseInt(clientUdpPort.getText().trim());
					PropertyManager.setProperty("CLIENT_UDP_PORT", UDP_PORT+1+"");
					clientUDP_PORT = UDP_PORT;
					
					netClient = new NetClient(TankClient.this, serverTCP_IP, serverUDP_PORT, clientUDP_PORT);
					...
					// connect to the server 
					netClient.connect(serverTCP_IP,TankServer.TCP_PORT);
