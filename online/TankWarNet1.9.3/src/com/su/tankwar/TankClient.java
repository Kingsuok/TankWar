...
/**
 *处理由于server 端的 socket的关闭，
 *处理 client的唯一标示问题：
 *server assigns every client a id, every client (tank) stores the id and sends message with the id to server 
 * because the assigned ID belongs to a Tank, so there is an attribute in Tank class.
 * @author su
 *
 */
public class TankClient extends Frame{
	...
	private void launchFrame(){
		...
		netClient.connect("192.168.0.11", TankServer.TCP_PORT);
